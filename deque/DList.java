...
public class DList<E extends Comparable<E>>{
    ...
    public void insertFront(E newItem){
        head = new DNode(newItem, head, tail);
        size++;
    }

    // exsitingNode 뒤에 새 노드를 삽입한다.
    public void insertAfter(E newItem, DNode exsitingNode){
        DNode newNode = new DNode(newItem, exsitingNode, exsitingNode.getNext());
    ...
    public void insertBefore(E newItem, DNode exsitingNode){
        DNode newNode = new DNode(newItem, exsitingNode.getPrevious(), exsitingNode);
...
//        if (cur.getItem() == target) {deleteFront(); return;}
    ...
    // 기존 노드 뒤에를 삭제한다.
    public void deleteAfter(DNode<E> exisit){
        if(exisit == null) throw new NoSuchElementException();

        DNode<E> exisitNext = exisit.getNext();
